...
namespace Saloon4kBLL
{
    public class DealsRepository
    {
        ...
        public int UpdateDeal(Deal deal)
        {
            ...
            using (var conn = new Saloon4KEntities())
            {
                ...
                if (newDeal != null)
                {
                    ...
                    newDeal.AcceptVouchers = deal.AcceptVouchers;
                }
                conn.SaveChanges();
                scoprId = deal.DealId;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public List<Deal> GetAllDeals(string country, int userId)
        {
            List<Deal> list;
            const string status = RecordStatus.Deleted;
            using (var conn = new Saloon4KEntities())
            {
                list = conn.Deals.Where(x => x.RecordStatus != status && x.Country == country && x.UserId == userId).OrderByDescending(y => y.DealId).ToList();
            }
            return list;
        }

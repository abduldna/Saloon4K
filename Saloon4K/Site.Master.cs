...
namespace Saloon4K
{
    public partial class Site : System.Web.UI.MasterPage
    {
        ...
        protected void btnForgotPassword_Click(object sender, EventArgs e)
        {
            try
            {
                var entity = UsersRepository.GetUserByEmail(txtEmailPassword.Text.Trim());
                if (entity != null)
                {
                    var password = Utilities.Helper.Decrypt(entity.Password);
                    SendEmailPassword(txtEmailPassword.Text.Trim(), password);
                    divMessageFP.Visible = true;
                    divMessageFP.InnerHtml = "Your password has been sent on your email address.";
                    divMessageFP.Attributes.Add("class", Common.success);
                }
                else
                {
                    divMessageFP.Visible = true;
                    divMessageFP.InnerHtml = "User with this email/username does not exist.";
                    divMessageFP.Attributes.Add("class", Common.error);
                }
            }
            catch (Exception ex)
            {
        ...
        public static bool SendEmailPassword(string receiverEmail, string password)
        {
            var mm = new MailMessage(ConfigurationManager.AppSettings["ADMIN_EMAIL"], receiverEmail, "Password Retrieval Request", MailBodyPassword(password))
            {
                IsBodyHtml = true
            };
            try
        ...
        public static string MailBodyPassword(string password)
        {

            var reader = new StreamReader(ConfigurationManager.AppSettings["SiteRoot"] + @"EmailTemplates\ForgetPassword.html");

                    ...
                    if (UserSession.Current.IsUserAuthenticated)
                    {
                        ...
                        if (p.Role != Common.Admin)
                        {
                            ...
                            if (isInterested == null)
                            {
                                ...
                                Utilities.Helper.InsertUserPoints(p.UserId, Common.SaloonInterested, Convert.ToInt32(e.CommandArgument), Utilities.Helper.GetPoints(Common.SaloonInterested));
                                var masterPage = Master;
                                if (masterPage != null)
                                {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(hfUserId.Value) && !string.IsNullOrEmpty(hfSaloonId.Value))
                {
                    ...
                    if (isBooked == null)
                    {
                        ...
                        Utilities.Helper.InsertUserPoints(p.UserId, Common.SaloonBooked, Convert.ToInt32(hfSaloonId.Value), Utilities.Helper.GetPoints(Common.SaloonBooked));
                        var masterPage = Master;
                        if (masterPage != null)
                        {
                            var divPoints = (HtmlGenericControl)masterPage.FindControl("divPoints");
